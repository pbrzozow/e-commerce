...
import com.ecommerce.shop.order.dto.RefundFailureException;
...
class PaymentService {
    private final PaymentPort paymentPort;
        ...
        PaymentResponse process(Order order, PaymentDetails paymentDetails) {
            PaymentRequest paymentRequest = new PaymentRequest(order.getId(),order.getOrderCart().getPrice(),paymentDetails);
